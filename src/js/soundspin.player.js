...
(function ($) {
	...
	soundspin.views.SoundSpinPlaylist = Backbone.View.extend({
        events: {
        },
        hasTrack: function(track) {
            return this.$('#'+track.id).length > 0;
        },
        addTrack: function(track, user) {
            this.$el.append(JTmpl.call('Track', {
                image:  (track.artwork_url ? track.artwork_url.replace(/large\.(\w{3})/,'small.$1') : ''),
                target: '#musician/' + user.id,
                track:  track
            }).data({
                user: user,
                track: track
